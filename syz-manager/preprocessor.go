...
func (p *PreprocessorBase) ParseCorpusToFile() {
	...
	for _, rec := range p.corpusDB.Records {
		...
		if programCount >= TokenFileSize {
			...
			SaveContentToFile(&buffer, VocabFilePath)
...
func (p *PAllocateConstant) Replace() {
	...
	for x, rec := range p.corpusDB.Records {
		...
		for i, call := range program.Calls {
			...
			GetAddrGeneratorInstance().ResetCounter()
			AddSyscallName(call.Meta.Name, call.Meta.CallName)
...
func SaveContentToFile(content *string, filePath string) {
...
func AddSyscallName(name string, nameWithoutDollar string) {
	syscallNames := GetSyscallNamesInstance()
	if _, exists := syscallNames.Names[nameWithoutDollar]; !exists {
		syscallNames.Names[nameWithoutDollar] = name
	}
}

func WriteSyscallNames() {
	const SyscallNamePath = "./syz-manager/data/names.txt"
	file, err := os.Create(SyscallNamePath)
	if err != nil {
		fmt.Println("Error creating addr.txt:", err)
		return
	}
	...
	syscallNames := GetSyscallNamesInstance()
	for key, value := range syscallNames.Names {
		line := fmt.Sprintf("%s %s\n", key, value)
		_, err := file.WriteString(line)
		if err != nil {
			fmt.Println("Error writing to names.txt:", err)
			return
		}
	}
}

type SyzcallNames struct {
	Names map[string]string
}

var _SyscallNamesInstance *SyzcallNames
var _SyscallNamesOnce sync.Once

func GetSyscallNamesInstance() *SyzcallNames {
	_SyscallNamesOnce.Do(func() {
		_SyscallNamesInstance = &SyzcallNames{
			Names: make(map[string]string),
		}
	})

	return _SyscallNamesInstance
}
